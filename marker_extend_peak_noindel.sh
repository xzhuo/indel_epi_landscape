...
perl -lane 'BEGIN{$,="\t"}{@map = splice @F,-6,6; print @F if($map[0] <= $map[3] && $map[2] > 0.7 && $map[4]>=$map[1]) || ($map[0] > $map[3] && $map[5] > 0.7 && $map[1]>=$map[4])}' all_peaks_on_both.collapse.${m}.mappability.txt > all_peaks_on_both.collapse.${m}.map_pass.txt
